...
# -------------------------------------
def build_all_indexes(data: np.ndarray, weight_list: np.ndarray) -> List[KDTree]:
    sqrt_weights = np.sqrt(weight_list)
    scaled_data_all = data[None, :, :] * sqrt_weights[:, None, :]
    kd_tree_list = [KDTree(scaled_data_all[i]) for i in range(scaled_data_all.shape[0])]
    return kd_tree_list
...
# 4. 저장 및 불러오기 (pickle)
# -------------------------------------
def save_forest(weight_list: np.ndarray, kd_tree_list: List[KDTree], filename: str = 'forest.pkl'):
    with open(filename, 'wb') as f:
        cloudpickle.dump((weight_list, kd_tree_list), f)
    print("Forest saved to", filename)

def load_forest(filename: str = 'forest.pkl') -> Tuple[Optional[np.ndarray], Optional[List[KDTree]]]:
    try:
...
def query_AkNN(q: np.ndarray, w_user: np.ndarray,
               weight_list: List[np.ndarray], kd_tree_list: List[KDTree],
               data: np.ndarray,
    ...
    if best_metric <= threshold:
        ...
        # 여기서 KDTree 대신, 원래 사용하던 KDTree (혹은 cKDTree)를 계속 사용할 수 있음.
        new_tree = KDTree(new_scaled_data)
    ...
    q_scaled = q_scaled.reshape(1, -1)
    distances, indices = chosen_tree.query(q_scaled, k=K)
    
    if K == 1:
        d = float(distances[0])
        idx = int(indices[0])
        neighbors = [(d, (chosen_tree.data[idx], idx))]
    else:
        distances = distances[0]  # 첫 번째 (유일한) query 결과 행 추출
        indices = indices[0]
        neighbors = [(d, (chosen_tree.data[idx], int(idx))) for d, idx in zip(distances, indices)]
...
def query_exactKNN(q: np.ndarray, w_user: np.ndarray, data: np.ndarray, K: int) -> List[tuple]:
    ...
    if K == 1:
        d = float(distances[0])
        idx = int(indices[0])
        neighbors = [(d, (tree.data[idx], idx))]
    else:
        distances = distances[0]
        indices = indices[0]
        neighbors = [(d, (tree.data[idx], int(idx))) for d, idx in zip(distances, indices)]
    return neighbors

# -------------------------------------
# 7. Classification: weighted voting based on distance
# -------------------------------------
def classify_knn_weighted(neighbors: List[tuple], labels: np.ndarray) -> int:
    epsilon = 1e-6
    vote_dict = {}
...
def measure_saving_time(weight_list: np.ndarray, kd_tree_list: List[KDTree], filename: str = 'forest.pkl') -> float:
    start = time.perf_counter()
...
def evaluate_queries(num_queries: int, labels: List[int], data: np.ndarray, 
                     weight_list: np.ndarray, kd_tree_list: List[KDTree], K: int) -> Tuple[float, float, float, float, float, float, float]:
    ...
    return avg_query_time, avg_exact_query_time, classification_accuracy, avg_query_error, avg_exact_matches, total_new_tree_count, threshold
...
if __name__ == "__main__":
    ...
    data_np, labels = fetch_kddcup99(download_if_missing=True, return_X_y=True)
    ...
    # RobustScaler와 StandardScaler를 사용해 데이터 표준화
    scaler = RobustScaler()
    ...
    #labels = decode(labels_np).tolist()
    ...
    num_queries = 200
    K = 20
    avg_q_time, avg_exact_q_time, class_acc, avg_q_err, avg_exact_matches, new_tree_count, threshold = evaluate_queries(num_queries, labels, data, weight_list, kd_tree_list, K)
