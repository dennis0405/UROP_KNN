...
# -------------------------------------
def classify_knn_weighted(neighbors: List[tuple], labels: List[str]) -> int:
    epsilon = 1e-6
    vote_dict = {}
...
def evaluate_queries(num_queries: int, labels: List[str], data: np.ndarray, 
                     weight_list: np.ndarray, kd_tree_list: List[cKDTree], K: int) -> Tuple[float, float, float, float, float, float, float]:
...
if __name__ == "__main__":
    ...
    num_initial_weights = len(weight_list)
